...
export const useCurrentAccountId = (): [ string|undefined, boolean, Error|null ] => {
  ...
  const { data: accountId, isLoading: isLoadingAccountId, isFetching: isFetchingAccountId, error } = useQuery<string, Error>({
  ...
  const isLoading = isLoadingAccountId || isFetchingAccountId;
