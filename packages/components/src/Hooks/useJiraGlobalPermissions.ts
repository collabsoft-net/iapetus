...
export const useJiraGlobalPermissions = (permissions: Array<string>, accountId?: string, mode: 'ALL'|'ANY' = 'ALL'): [ boolean|undefined, boolean, Error|undefined ] => {
  ...
  useEffect(() => {
    if (!isLoadingContext && !isLoadingUser) {
      ...
      if (!context) {
        ...
        service.hasPermissions(accountId, undefined, permissions, mode)
