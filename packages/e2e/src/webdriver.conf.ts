...
// import { reportResult } from './shared/confluence';
load(join(process.cwd(), '../../'));

import * as commands from './commands';

export const getBaseConfig = (baseUrl: string): WebdriverIO.Config => ({
    //
    // ====================
    ...
    capabilities: [{
        ...
        'goog:chromeOptions': {
            args: process.env.HEAD === 'true' ? ['--disable-gpu', '--window-size=1280,800']
    ...
    onPrepare: function () {
        console.log(`Starting e2e tests against Atlassian instance ${baseUrl}`);
    },
    /**
    ...
    before: function (_capabilities, _specs, browser) {
        browser.addCommand('open', (url: string, waitForElement?: string|Array<string>, timeout?: number): Promise<void> => commands.open(url, waitForElement, timeout));
    ...
    afterTest: function(test, _context, result) {
        if (!result.passed) {
            console.log(`Test ${test.title} failed`, result);
            browser.saveScreenshot(test.title);
        }
    },
    /**
    ...
    after: async function (result) {
        console.log('Reporting test results to Atlassian');
        for await (const appKey of Object.values(AppKeys)) {
            const status = result === 0 ? 'PASSED' : 'FAILED';
            console.log(`Sending ${status} result for ${appKey}`);
            await reportResult(appKey, result === 0);
        }
    },
    */
