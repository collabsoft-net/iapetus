...
export class ForgeRestClient implements RestClient {
  ...
  protected async request<T>(method: RestClientMethods, endpoint: string, data?: unknown, params?: Record<string, string|number|boolean|undefined>, config?: AxiosRequestConfig, cacheDuration?: number): Promise<AxiosResponse<T>> {
    ...
    const appOrUserAPI = this._accountId ? api.asUser(this._accountId) : api.asApp();
    const client = this.product === 'jira' ? appOrUserAPI.requestJira : this.product === 'confluence' ? appOrUserAPI.requestConfluence : appOrUserAPI.requestBitbucket;
    ...
    const body = 
      method === RestClientMethods.GET || method === RestClientMethods.HEAD
        ? undefined
        : data || config?.data ? JSON.stringify(data || config?.data) : undefined;

    const fetchFromRemote = async (): Promise<AxiosResponse<T>> => client(route`${endpoint}`, { method, headers, body })
  ...
  private async toAxiosResponse<T>(response: APIResponse, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> {
